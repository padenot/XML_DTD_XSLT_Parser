...
int LexerAdvice(int line, char c) {
	...
	if(c == '"' || c == '\'') fprintf(stderr, " Unclosed attribut string value, %c expected.\n", c);
	else printf( "\n" );
...
int noAttributeValue(const char* attributName) {
	fprintf(stderr, "\tSyntax error ( line %d ) : No value for attribut \'%s\'\n", xmllineno, attributName);
	return 1;
}

int UnmatchedNames(const char* openingName, const char* closingName) {
	if ( strcmp( openingName, closingName) != 0 ) {
		fprintf(stderr, "\tSyntax error ( line %d ) : Opening & closing markup name don't match ( %s != %s ), maybe you forgot to close the markup ?\n", xmllineno, openingName, closingName);
		return 1;
	}
